...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['tailor', 'business-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<TailorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/tailors` : null),
    () => getTailors({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Tailor',
      path: isTenantUser ? `/tailors/view/${data?.[0]?.id}` : '/tailors',
      entity: 'tailor',
      service: AccessServiceEnum.PROJECT,
      icon: FiScissors,
    },
    { name: 'Garment', path: '/garments', entity: 'garment', service: AccessServiceEnum.PROJECT, icon: FiTrello },
